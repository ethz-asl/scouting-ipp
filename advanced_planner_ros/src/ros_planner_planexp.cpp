...
namespace active_3d_planning {
  namespace ros {
    ...
    RosPlannerPlanExp::RosPlannerPlanExp(const ::ros::NodeHandle& nh,
      ...
      odom_sub_ = nh_.subscribe("odometry", 1, &RosPlannerPlanExp::odomCallback, this);
      get_cpu_time_srv_ = nh_private_.advertiseService(
          "get_cpu_time", &RosPlannerPlanExp::cpuSrvCallback, this);

      // Finish
      ROS_INFO_STREAM(
          "\n******************** Initialized Planner ********************\n"
          "Initialized 'RosPlannerPlanExp' from namespace '"
              << param_map->at("param_namespace")
              << "' with parameters:" << param_map->at("verbose_text"));
    }

    void RosPlannerPlanExp::setupFromParamMap(Module::ParamMap* param_map) {
    ...
    void RosPlannerPlanExp::setupFactoryAndParams(ModuleFactory* factory,
    ...
    void RosPlannerPlanExp::initializePlanning() {
      // setup standard
    ...
    void RosPlannerPlanExp::planningLoop() {
      ...
      run_srv_ = nh_private_.advertiseService("toggle_running",
                                              &RosPlannerPlanExp::runSrvCallback, this);
      running_ = true;
      std::clock_t timer;
      while (::ros::ok()) {
        if (planning_) {
    ...
    bool RosPlannerPlanExp::requestNextTrajectory() {
    ...
    void RosPlannerPlanExp::odomCallback(const nav_msgs::Odometry& msg) {
      ...
      if (running_ && !target_reached_) {
        ...
        if (p_replan_pos_threshold_ <= 0 ||
          ...
          if (p_replan_yaw_threshold_ <= 0 ||
            ...
            map_->targetReachedCallback();
          }
        }
      }
    }

    void RosPlannerPlanExp::requestMovement(const EigenTrajectoryPointVector& trajectory) {
      if (trajectory.empty()) {
    ...
    void RosPlannerPlanExp::publishVisualization(const VisualizationMarkers& markers) {
    ...
    bool RosPlannerPlanExp::runSrvCallback(std_srvs::SetBool::Request& req,
    ...
    bool RosPlannerPlanExp::cpuSrvCallback(std_srvs::SetBool::Request& req,
                                    std_srvs::SetBool::Response& res) {
      double time =
...
// logging and printing
    void RosPlannerPlanExp::printInfo(const std::string& text) { ROS_INFO_STREAM(text); }

    void RosPlannerPlanExp::printWarning(const std::string& text) {
      ROS_WARN_STREAM(text);
    }

    void RosPlannerPlanExp::printError(const std::string& text) { ROS_ERROR_STREAM(text); }
